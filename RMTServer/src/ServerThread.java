	...
	public void run() {
		try {
			controlInputStream = new BufferedReader(new InputStreamReader(
					clientSocketControl.getInputStream()));
			controlOutputStream = new PrintStream(
					clientSocketControl.getOutputStream());

			while (true) {
				if (start) {
					controlOutputStream.println("Welcome to WEB calculator.");
					controlOutputStream
							.println("Valid operations are: addition(+), substriction(-), division(/) and multiplication(*).");
					start = false;
				}
				controlOutputStream
						.println("Please choose the arithmetic operation:");
				...
				if (operationFromClient.equals("+")
						|| operationFromClient.equalsIgnoreCase("addition")) {
					controlOutputStream
							.println("Choosen operation is: ADDITION.");
					operationToDataThread = "+";
				} else if (operationFromClient.equals("-")
						|| operationFromClient.equalsIgnoreCase("subtraction")) {
					controlOutputStream
							.println("Choosen operation is: SUBTRACTION.");
					operationToDataThread = "-";
				} else if (operationFromClient.equals("*")
						|| operationFromClient
								.equalsIgnoreCase("multiplication")) {
					controlOutputStream
							.println("Choosen operation is: MULTIPLICATION.");
					operationToDataThread = "*";
				} else if (operationFromClient.equals("/")
						|| operationFromClient.equalsIgnoreCase("division")) {
					controlOutputStream
							.println("Choosen operation is: DIVISION.");
					operationToDataThread = "/";
				} else {
				...
				controlOutputStream
						.println("Enter numbers with space between them:");
				// podaci
				Socket clientSocketData = dataSocket.accept();
				DataThread dataThread = new DataThread(clientSocketData,
						operationToDataThread);
				dataThread.start();
